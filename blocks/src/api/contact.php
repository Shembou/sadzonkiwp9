...
class contact {
    public static function register_contact_routes() {
        register_rest_route('blocks/v1', '/contact-min', array(
            'methods'  => 'POST',
            'callback' => [self::class, 'handle_minimal_form_submission'],
            'permission_callback' => '__return_true',
        ));
        register_rest_route('blocks/v1', '/contact', array (
            'methods'  => 'POST',
            'callback' => [self::class, 'handle_form_submission'],
            'permission_callback' => '__return_true',
        ));
    }
    ...
    public static function handle_form_submission(\WP_REST_Request $request) {
        ...
        if (empty($text) || empty($name) || empty($email) || empty($tel)) {
    ...
    public static function handle_minimal_form_submission(\WP_REST_Request $request) {
        $text   = sanitize_textarea_field($request->get_param('text'));
        $name   = sanitize_text_field($request->get_param('name'));
        $email  = sanitize_email($request->get_param('mail'));
        $policy = $request->get_param('privacy-policy');
